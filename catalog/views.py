...
def showCategory(category_id):
	...
	items_popular =session.query(Item).filter_by(category_id=category_id).order_by(desc(Item.no_of_visits))[0:8]
	return render_template('category.html', category_items=category_items, category=category, items_popular=items_popular)
...
def createNewCategory():
	...
	if request.method == 'POST':
		newCategory = Category(name= request.form['name'], no_of_visits=1, user_id=login_session['user_id'])
...
def editCategory(category_id):
	...
	if 'username' not in login_session or creator.id !=login_session['user_id']:
		...
		return render_template('category_public.html', category_items=category_items, category=editedCategory, items_popular=items_popular)
...
def deleteCategory(category_id):
	...
	#Objects needed for none authorized users
	creator = getUserInfo(deletedCategory.id)
	category_items = session.query(Item).filter_by(category_id= category_id)
	items_popular =session.query(Item).filter_by(category_id=category_id).order_by(desc(Item.no_of_visits))[0:8]
	...
	if 'username' not in login_session or creator.id !=login_session['user_id']:
		...
		return render_template('category_public.html', category_items=category_items, category=deletedCategory, items_popular=items_popular)
	...
	try:
		if request.method == 'POST':
			...
			return render_template('deletedCategory.html', category=deletedCategory)
...
def editItem(category_id, item_id):
	...
	if 'username' not in login_session or creator.id !=login_session['user_id']:
		...
		return render_template('item_public.html', category=category, item=item)
...
def deleteItem(category_id, item_id):
	...
	if 'username' not in login_session or creator.id !=login_session['user_id']:
		if deletedItem.no_of_visits is None:
			...
			session.commit()
		flash("You must login to edit an item or be the creator of the item.")
		return render_template('item_public.html', category=category, item=deletedItem)
...
def newItem(category_id):
	...
	category = session.query(Category).filter_by(id=category_id).one()
	if request.method =='POST':
		newItem = Item(name= request.form['name'], description= request.form['description'], picture_1= request.form['picture_1'], picture_2=request.form['picture_2'], picture_3= request.form['picture_3'], picture_4=request.form['picture_4'], category_id=category_id, user_id=login_session['user_id'])
		session.add(newItem)
